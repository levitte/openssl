...
static int kdf_hkdf_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if ((p = OSSL_PARAM_locate_const(params, OSSL_KDF_PARAM_MODE_INT))
        != NULL) {
        /* Numeric modes to be deprecated eventually */
        if (got_mode) {
            ERR_raise(ERR_LIB_PROV, PROV_R_BOTH_MODE_AND_MODE_INT);
            return 0;
        }
        ...
        if (n != EVP_KDF_HKDF_MODE_EXTRACT_AND_EXPAND
            ...
            ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_MODE_INT);
            return 0;
        }
        ctx->mode = n;
    }
...
static const OSSL_PARAM *kdf_hkdf_settable_ctx_params(void)
{
    static const OSSL_PARAM known_settable_ctx_params[] = {
        ...
        OSSL_PARAM_int(OSSL_KDF_PARAM_MODE_INT, NULL),

...
static OSSL_OP_mac_ctx_set_param_types_fn cmac_set_param_types;
static OSSL_OP_mac_ctx_set_params_fn cmac_set_params;
static OSSL_OP_mac_size_fn cmac_size;
...
static const OSSL_PARAM *cmac_set_param_types(void)
{
    return known_params;
}
 ...
 */
static int cmac_set_params(void *vmacctx, const OSSL_PARAM params[])
{
    struct cmac_data_st *macctx = vmacctx;
    const OSSL_PARAM *p;
    ...
    if ((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_CIPHER)) != NULL
...

        {
            ...
            const OSSL_PARAM *p_eng =
                OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_ENGINE);
            const OSSL_PARAM *p_pq =
                OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_PROPERTIES);
            ...
            if (p_eng != NULL) {
                if (p_eng->data_type != OSSL_PARAM_UTF8_STRING)
                    return 0;

                macctx->tmpengine = ENGINE_by_id(p_eng->data);
                if (macctx->tmpengine == NULL)
                    return 0;
            }
            if (p_pq != NULL) {
                if (p_pq->data_type != OSSL_PARAM_UTF8_STRING)
                    return 0;
...
const OSSL_DISPATCH cmac_functions[] = {
    ...
    { OSSL_FUNC_MAC_SIZE, (void (*)(void))cmac_size },
    { OSSL_FUNC_MAC_INIT, (void (*)(void))cmac_init },
    { OSSL_FUNC_MAC_UPDATE, (void (*)(void))cmac_update },
    { OSSL_FUNC_MAC_FINAL, (void (*)(void))cmac_final },
    { OSSL_FUNC_MAC_CTX_SET_PARAM_TYPES, (void (*)(void))cmac_set_param_types },
    { OSSL_FUNC_MAC_CTX_SET_PARAMS, (void (*)(void))cmac_set_params },
    { 0, NULL }
};

...
static int mac_test_run_mac(EVP_TEST *t)
{
    ...
    const OSSL_PARAM *defined_params =
        EVP_MAC_CTX_set_param_types(expected->mac);
    ...
    if (expected->alg != NULL)
        params[params_n++] =
            OSSL_PARAM_construct_utf8_string("algorithm", expected->alg,
                                             strlen(expected->alg) + 1);
    if (expected->key != NULL)
        params[params_n++] =
            OSSL_PARAM_construct_octet_string("key", expected->key,
                                              expected->key_len);
    if (expected->custom != NULL)
        params[params_n++] =
            OSSL_PARAM_construct_octet_string("custom", expected->custom,
                                              expected->custom_len);
    if (expected->salt != NULL)
        params[params_n++] =
            OSSL_PARAM_construct_octet_string("salt", expected->salt,
                                              expected->salt_len);
    if (expected->iv != NULL)
        params[params_n++] =
            OSSL_PARAM_construct_octet_string("iv", expected->iv,
                                              expected->iv_len);
    ...
    t->err = NULL;
 err:
    for (; params_n-- > params_n_allocstart;) {

...
use Test::More;
use lib 'testlib';
use OpenSSL::Test qw/setup indir run app test top_file top_dir/;
...
sub print_request {
    my $input = shift;
    return
	ok(run(app(["openssl", "ts", "-query", "-in", $input, "-text"])));
}
...
sub create_time_stamp_request3 {
    ...
    return
	ok(run(app(["openssl", "ts", "-query", "-data", "../CAtsa.cnf", "-no_nonce", "-out", "req3.tsq"])))
}

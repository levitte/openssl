...
use Test::More;
use lib 'testlib';
use OpenSSL::Test qw/setup with app test pipe run/;

setup("test_cms");

$ENV{OPENSSL_CONF} = devnull();
my $ocspdir="ocsp-tests";
...
my @smime_cms_tests = (
    ...
    [ "signed content MIME format, RSA key, signed receipt request",
      [ "-sign", "-in", "smcont.txt", "-signer", catfile($smdir, "smrsa1.pem"), "-nodetach",
...
subtest "CMS => PKCS#7 compatibility tests\n" => sub {
    ...
    foreach (@smime_pkcs7_tests) {
	  ...
	  ok(run(app(["openssl", "cms", @{$$_[1]}]))
	     ...
	     && compare("smcont.txt", "smtst.txt") == 0,
	     $$_[0]);
	}
    }
};

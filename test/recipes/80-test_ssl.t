...
use Test::More;
use lib 'testlib';
use OpenSSL::Test qw/setup with run app test/;
...
subtest 'test_ss' => sub {
    if (testss()) {
	open OUT, ">", "intP1.ss";
	copy("certCA.ss", \*OUT); copy("certU.ss", \*OUT);
	close OUT;

	open OUT, ">", "intP2.ss";
	copy("certCA.ss", \*OUT); copy("certU.ss", \*OUT); copy("certP1.ss", \*OUT);
	close OUT;
    }
};

subtest 'test_ssl' => sub {
    plan tests => 4;
  ...
  SKIP: {
      ...
      subtest 'test_ssl -- key U' => sub {
	  testssl("keyU.ss", "certU.ss", "certCA.ss");
      };
      subtest 'test_ssl -- key P1' => sub {
	  testsslproxy("keyP1.ss", "certP1.ss", "intP1.ss");
      };
      subtest 'test_ssl -- key P2' => sub {
...
sub testss {
    ...
    my $dummycnf = File::Spec->catfile("..", "apps", "openssl.cnf");
...
sub testssl {
    my $key = shift || "../apps/server.pem";
    my $cert = shift || "../apps/server.pem";
    my $CAtmp = shift;
    my @CA = $CAtmp ? ("-CAfile", $CAtmp) : ("-CApath", "../certs");
    my @extra = @_;
	...
	plan tests => 27 + $protocolciphersuitcount;
...

	{
	  SKIP: {
		  ...
		  if (run(app(["openssl", "no-dh"], stdout => undef)));
...

	{
	  SKIP: {
	      ...
	      ok(run(test(["ssltest", "-v", "-bio_pair", "-tls1", "-cert", "../apps/server2.pem", "-no_dhe", "-no_ecdhe", "-num", "10", "-f", "-time", @extra])),
		 'test tlsv1 with 1024bit RSA, no (EC)DHE, multiple handshakes');

	      skip "skipping RSA+DHE tests", 1
		  if (run(app(["openssl", "no-dh"], stdout => undef)));

	      ok(run(test(["ssltest", "-v", "-bio_pair", "-tls1", "-cert", "../apps/server2.pem", "-dhe1024dsa", "-num", "10", "-f", "-time", @extra])),
...
sub testsslproxy {
	...
	foreach my $cond (('A', 'B', 'C', 'A|B&!C')) {
	    ...
	    with({ exit_checker => sub { return shift == 3 ? 0 : 1; } },
		 sub {
		     subtest "auth $auth, cond $cond" => sub {
			 testssl($a1, $a2, $a3,

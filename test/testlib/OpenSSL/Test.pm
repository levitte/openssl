...
sub setup {
    ...
    BAIL_OUT("Need \$OPENSSL_SRCDIR") if (! $ENV{OPENSSL_SRCDIR});

    my $srcdir = $ENV{OPENSSL_SRCDIR};
...
####
####    # Unix setup
####    my $failure_code = 256;	# 1 << 8
####    my $arg_str = "";
####    $arg_str = " '".join("' '", @args)."'" if @args;
####    my $null = devnull();
####
####    # VMS setup
####    if ( $^O eq "VMS") {
...
####    my $stdout = "";
####    my $stderr = "";
####    $stdin = " < ".$fileornull->($options{stdin})  if exists($options{stdin});
####    $stdout= " > ".$fileornull->($options{stdout}) if exists($options{stdout});
####    $stderr=" 2> ".$fileornull->($options{stderr}) if exists($options{stderr});
####
####    my $errlog = "$test_name.tmp_err";
...
####    close ERR;
####
####    copy($errlog,$options{stderr}) if defined($options{stderr});
####    unlink($errlog);
####
####    return $r;
####}
####
####sub run_app {
####    my $app = __app_cmd(shift @{$_[0]});
####    my @args = @{$_[0]};
####    shift;
####
####    return $app ? __run([$app,@args],@_) : 0;
####}
####sub run_test {
####    my $test = __test_cmd(shift @{$_[0]});
####    my @args = @{$_[0]};
####    shift;
####
####    return $test ? __run([$test,@args],@_) : 0;
####}
####
####sub pipe_app {
####    my $app = __app_cmd(shift @{$_[0]});

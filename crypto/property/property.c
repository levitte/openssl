...
static void alg_cleanup(ossl_uintmax_t idx, ALGORITHM *a)
{
    if (a != NULL) {
        ...
        OPENSSL_free(a);
        fprintf(stderr, "DEBUG[%s]:           algorithm 0x%x removed\n",
                __func__, nid);
    }
}
...
{
    if (store != NULL) {
        fprintf(stderr, "DEBUG[%s]: method store %p: cleanup started\n",
                __func__, (void *)store);
        ossl_sa_ALGORITHM_doall(store->algs, &alg_cleanup);
        ossl_sa_ALGORITHM_free(store->algs);
        fprintf(stderr, "DEBUG[%s]: method store %p: cleanup finished\n",
                __func__, (void *)store);

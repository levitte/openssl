...
static void depopulate_queue(struct ossl_event_queue_st *queue)
{
    ...
    if (queue->events_stacks != NULL) {
        size_t i;

        for (i = 0; i < queue->events_stacks->priorities; i++)
            sk_OSSL_EVENT_pop_free(queue->events_stacks->events[i],
                                   ossl_event_free);
        OPENSSL_free(queue->events_stacks);
    }
}
...
static int populate_events_stacks(struct ossl_event_queue_st *queue,
                                  size_t priorities)
{
    size_t events_stacks_size = 0;
    size_t i;

    if (priorities == 0)
        return 1;

    events_stacks_size += sizeof(*queue->events_stacks);
    events_stacks_size += (sizeof(*queue->events_stacks->events)
                           * (priorities - 1));
    if ((queue->events_stacks = OPENSSL_zalloc(events_stacks_size)) == NULL)
        return 0;
    queue->events_stacks->priorities = priorities;
    for (i = 0; i < priorities; i++)
        if ((queue->events_stacks->events[i] = sk_OSSL_EVENT_new_null())
            == NULL)
            return 0;
    return 1;
}
...
int ossl_event_queue_dispatch(struct ossl_event_queue_st *queue)
{
    ...
    for (i = 0; i < queue->events_stacks->priorities; i++) {
        int s = sk_OSSL_EVENT_num(queue->events_stacks->events[i]);

        if (s > 0) {
            int ret;
            OSSL_EVENT *event
                = sk_OSSL_EVENT_shift(queue->events_stacks->events[i]);
                         ...
                         struct ossl_event_st *event)
{
    ...
    if (queue == NULL || queue->events_stacks == NULL)
        return 0;                /* Not added */
    ...
    return
        sk_OSSL_EVENT_push(queue->events_stacks->events[priority], event) > 0
        ?  1                     /* Added */
        : -1                     /* Error */
        ;
}
                            ...
                            void *filter_data)
{
    ...
    for (i = 0; i < queue->events_stacks->priorities; i++) {
        ...
        end = sk_OSSL_EVENT_num(queue->events_stacks->events[i]);
        for (j = end; i-- > 0;) {
            OSSL_EVENT *e
                = sk_OSSL_EVENT_value(queue->events_stacks->events[i], j);
            if (filter(e, filter_data)) {
                sk_OSSL_EVENT_delete(queue->events_stacks->events[i], j);
                ossl_event_free(e);
                count_removed++;
            }
        }
                              ...
                              void *subscriber_data)
{
    ...
    if (ossl_event_queue_subscribe(queue, waiter, &data) < 0)
        return -1;
                                      ...
                                      void *subscriber_data)
{
    ...
    case 0:     /* Event not added, dispatch directly */
        {
            ...
            data.subscriber_data = subscriber_data;
            data.done = 0;
            if (!ossl_event_queue_subscribe(queue, waiter, &data))
                return -1;           /* Error */
            ret = perform_dispatch(queue, event);
            ossl_event_queue_unsubscribe(queue, waiter, &data);
            if (ret >= 0)
    ...
    case 1:     /* Event added, use ossl_event_queue_wait_for() */
        ret = ossl_event_queue_wait_for(queue, subscriber, subscriber_data);
        break;
    }
    return ret;
}
